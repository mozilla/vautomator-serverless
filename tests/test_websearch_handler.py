...
class TestWebSearchHandler():
    ...
    def test_creation(self):
        ...
        assert type(websearch_handler) is WebSearchHandler

    def test_queue(self, sqs):
        client, queue_url = sqs
        test_event = {"body": '{"target": "infosec.mozilla.org"}'}
        test_context = None
        websearch_handler = WebSearchHandler(client, queue_url)
        websearch_handler.queue(test_event, test_context)
        response = client.receive_message(QueueUrl=queue_url)
        scan_type, target, uuid = response['Messages'][0]['Body'].split('|')
        assert scan_type == "websearch"
